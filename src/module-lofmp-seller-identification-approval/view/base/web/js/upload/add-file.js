...
define([
    'jquery',
    'mage/template',
    'text!Lofmp_SellerIdentificationApproval/template/upload/files.html',
    ...
    $.widget('mage.addFile', {
        options: {
            wrapFiles: '[data-role="driving_license-added-files"]',
        ...
        _create: function () {
            ...
            if (!$(this.element).parents('.field-attachment').hasClass('_disabled')) {
                this.options.parent = $(this.element).parents('.field-attachment');
                this._bind();
            } else {
        ...
        _addFiles: function (data) {
            ...
            var wrapper = $(this.element).closest('[data-role="attach-wrapper"]');
            wrapper.find(".added-files").append(file.last());
            this.element.hide();
            $(this.element).closest('.field-attachment').trigger('add', {
                data: data,
