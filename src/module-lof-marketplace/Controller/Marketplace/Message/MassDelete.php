...
use Lof\MarketPlace\Model\ResourceModel\Message\CollectionFactory;
...
class MassDelete extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        if ($customerSession->isLoggedIn() && $status == 1) {
            ...
            $collection->addFieldToFilter("owner_id", (int)$sellerId);
            $i=0;
            $messageIds = [];
            /** delete mesasge listing */
            foreach ($collection as $item) {
                if ((int)$item->getOwnerId() == (int)$sellerId) {
                    $i++;
            ...
            $detailCollection = $this->detailCollectionFactory->create()
                                ->addFieldToFilter("message_admin", 0)
            ...
            $this->_redirect('catalog/message/index');
