...
class Address extends \Lof\MarketPlace\Block\Seller\AbstractBlock
{
    ...
    protected function _prepareForm()
    {
        ...
        $form->setAction($this->getUrl('*/*/saveaddress', ['section' => AbstractModel::SHIPPING_SECTION]))
            ->setId('form-validate')
            ->setMethod('POST')
            ->setEnctype('multipart/form-data')
            ->setUseContainer(true);
        $seller = $this->getSeller();
        $model = $this->_objectManager->get(\Lof\MarketPlace\Model\Config\Shipping\Address::class);
        $code = 'address';
        $fields = $model->getFields();
        if (count($fields) > 0) {
            $key_tmp = $this->_objectManager->get(\Lof\MarketPlace\Helper\Data::class)->getTableKey('key');
            $seller_id_tmp = $this->_objectManager->get(\Lof\MarketPlace\Helper\Data::class)->getTableKey('seller_id');
    ...
    public function getRegionId()
    {
        ...
        $addressModel = $this->_objectManager->create(\Lof\MarketPlace\Model\Config::class)
            ->loadByField([$key_tmp, $seller_id_tmp], [$key, (int)$this->getSeller()->getId()]);
