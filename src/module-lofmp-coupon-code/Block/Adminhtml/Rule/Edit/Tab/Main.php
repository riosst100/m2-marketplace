...
class Main extends \Magento\Backend\Block\Widget\Form\Generic implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField(
            ...
            [
                'label'    => __('Seller'),
                'title'    => __('Coupon rule for seller'),
                'name'     => 'seller_id',
                'options'  => $this->getSellerList(),
                'defaule'  => 'Please select seller',
                'disabled' => $isElementDisabled
            ]
        );
        if (!$model->getId()) {
    ...
    public function getSellerList()
    {
        ...
        $data          = $sellerModel->getCollection()->getData();
        $sellerList    = [];
        if(count($data) > 0)
            $sellerList[0] = __("Please select seller");
        foreach ($data as $value) {
            $sellerList[$value["seller_id"]] = $value["name"];
        }
        return $sellerList;
    }
}
