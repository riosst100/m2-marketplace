...
*/
        ...
        $productModel  = $objectManager->get("Magento\Catalog\Model\ProductRepository");
        $couponModel   = $objectManager->get("Lofmp\CouponCode\Model\Coupon");
        $couponData    = $couponModel->getCollection()->getByCouponCode($quote->getCouponCode());
        $sellerProductList    = [];
        if(count($couponData) > 0 && isset($couponData["seller_id"])){
            $sellerProductModel = $objectManager->get("Lof\MarketPlace\Model\SellerProduct")->getCollection();
            $products           = $sellerProductModel->addFieldToFilter("seller_id", $couponData["seller_id"])->getData();
            foreach ($products as $product) {
                $sellerProductList[] = $product["product_id"];
            } 
        }
        $checkCouponSeller = $couponModel->getCollection()->addFieldToFilter("code", $quote->getCouponCode());
/*
    END CUSTOM
*/
        ...
        foreach ($items as $item) { 
            if(count($checkCouponSeller->getData()) > 0){
                $sku = $item->getSku();
                $itemId = $productModel->get($sku)->getId();
                if ($item->getNoDiscount() || !$this->calculator->canApplyDiscount($item) || !in_array($itemId, $sellerProductList)) {
