    ...
    protected function processControllerAction($seller = null) 
    {
        ...
        if ($id) {
            ...
            try {
                ...
                if ($quoteModel->getStatus() == \Lof\Quickrfq\Model\Quickrfq::STATUS_DONE || $quoteModel->getStatus() == \Lof\Quickrfq\Model\Quickrfq::STATUS_CLOSE) {
                    ...
                    $this->_eventManager->dispatch(
                        'seller_quickrfq_on_renew',
                        ['contact_name' => $contact_name, 'quote' => $quoteModel, 'status' => 'success']
                    );
                } else {
                    $this->messageManager->addError(__('Can not re-new the processing quote.'));
                }
