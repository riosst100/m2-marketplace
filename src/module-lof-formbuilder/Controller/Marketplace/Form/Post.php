...
namespace Lof\Formbuilder\Controller\Marketplace\Form;
...
$this->mappingHelper = $objectManager->create(\TCGCollective\ProductAttributesLink\Helper\Data::class);
                ...
                $val = $this->getRequest()->getParam('loffield_' . $code . $form->getId());
                ...
                if ($code && $val) {
                    $val = $val && $val != null ? ucwords($val) : '';
                    
                    $optionId = $this->mappingHelper->getAttributeOptionId($code, $val);
                    if ($optionId) {
                        
                        $newOptions = [];

                        $options = $this->mappingHelper->getAttributeOptions($code);
                        if ($options) {
                        ...
                        $attributeResponse['selected_option'] = $optionId;
                        $attributeResponse['options'] = $newOptions;
                    }
                }

                $responseData['attribute'] = $attributeResponse;
