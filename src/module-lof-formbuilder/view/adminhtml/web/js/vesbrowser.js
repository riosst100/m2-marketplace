            ...
            $.widget("mage.mediabrowser", {
                ...
                insert: function (event) {
                    ...
                    return $.ajax({
                        ...
                        if ($('.response-field-image.editing .field-img-preview').length) {
                            $('.response-field-image.editing .field-img-preview').attr("src", data);
                            var cid = $(targetEl).parents('#container').find('.custom-field-image-id').attr('id');
                            var data_form = JSON.parse($('#form-design').val());
                            if (!data_form.hasOwnProperty('fields')) {
                                data_form = {
                                    "fields": data_form
                                }
                            }
                            for (let index = 0; index < data_form.fields.length; index++) {
                                if (data_form.fields[index].cid === cid) {
                                    data_form.fields[index].image_url = data;
                                }
                            }
                            $('#form-design').val(JSON.stringify(data_form));
                            $('#form-design').html(JSON.stringify(data_form));
                        }
                        VesMediabrowserUtility.closeDialog();
                        targetEl.focus();
                    }, this));
