...
/**
 * A model for a seller tree structure entity. Used for work with seller inner hierarchy.
 */
class Structure
{
    /**
     * @var \Lof\MarketPermissions\Model\ResourceModel\Structure\Tree
     */
    private $tree;

    /**
     * @var \Lof\MarketPermissions\Api\Data\StructureInterfaceFactory
     */
    ...
    /**
     * @param \Lof\MarketPermissions\Model\ResourceModel\Structure\Tree $tree
     * @param \Lof\MarketPermissions\Api\Data\StructureInterfaceFactory $structureFactory
    ...
    public function __construct(
        \Lof\MarketPermissions\Model\ResourceModel\Structure\Tree $tree,
    ...
    /**
     * Adds seller member and team data to tree. Used for displaying seller hierarchy tree.
     *
                ...
                if ($item->getData(StructureInterface::ENTITY_TYPE) == StructureInterface::TYPE_CUSTOMER) {
                    foreach ($customers as $key => $customer) {
                        ...
                        if ($customer->getId() == $item->getData(StructureInterface::ENTITY_ID)) {
                            ...
                            $isActive = $sellerAttributes->getStatus() == SellerCustomerInterface::STATUS_ACTIVE;
    ...
    /**
     * Moves teams and users within the seller structure.
     *
     * @param int $id
    ...
    /**
     * Execute moving of teams and users within the seller structure.
     *
     * @param int $id
     * @param int $newParentId
    ...
    private function checkIfNodeMoveIsPossible(
        ...
        if ($changeSuperUser === false && !$node->getData(StructureInterface::PARENT_ID)) {
            throw new LocalizedException(__(
                'The seller admin cannot be moved to a different location in the seller structure.'
            ));
        }
        ...
        if (!$changeSuperUser) {
            ...
            $this->treeWalk(
                $tree,
                function (\Magento\Framework\Data\Tree\Node $childNode) use ($newParent, &$isSellerNode) {
                    if ($newParent->getId() == $childNode->getId()) {
                        $isSellerNode = true;
                    }
                }
            ...
            if (!$isSellerNode) {
                throw new LocalizedException(__(
                    'The specified parent ID belongs to a different seller.'
                    . ' The specified entity (team or user) and its new parent must belong to the same seller.'
                ));
            }
        }
    }
    ...
    /**
     * Removes customer node. Used when a customer or a whole seller is deleted.
     *
     ...
     * @return \Lof\MarketPermissions\Api\Data\StructureInterface|null
     * @throws \Magento\Framework\Exception\NoSuchEntityException
     */
    private function getTargetStructure(\Lof\MarketPermissions\Api\Data\StructureInterface $structure)
    {
        if ($structure) {
            $entityType = $structure->getEntityType();
            if ($entityType == \Lof\MarketPermissions\Api\Data\StructureInterface::TYPE_TEAM) {
                return $structure;
            } elseif ($entityType == \Lof\MarketPermissions\Api\Data\StructureInterface::TYPE_CUSTOMER &&
